...
def main():
    # 0. 清理旧日志 (可选，防止 TensorBoard 混乱)
    # 如果你不想手动删 logs 文件夹，可以保留这行
    print(">>> 正在初始化...")
    ...
    model = DQN(
        ...
        buffer_size=100000,     # 加大记忆库，记住更多"吃鬼"的瞬间
        learning_starts=5000,   # 先随机跑5000步，填满一点记忆再开始训练
        batch_size=128,         # 每次提取更多样本来学习
        gamma=0.99,             # 重视未来奖励
        train_freq=4,           # 每4步训练一次
        target_update_interval=1000, # 网络更新频率
        exploration_fraction=0.3, # 前 40% 的时间都在逐渐减少随机探索
        exploration_initial_eps=1.0,
        exploration_final_eps=0.05, # 最后保持 5% 的随机性
    )
